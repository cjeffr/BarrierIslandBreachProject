...
class RandomBreach(): 
    ...
    def combine_data(self, data):
        ...
        self.breach_data = self.breach_data.drop(columns=list(data.keys()))
...
def runit(num_sims):
    ...
    breach_data_path = os.path.join(PATH, '486/depth_width_sensitivity/vary_depth/d1')
    gauge_location_path = os.path.join(PATH, 'ocean_gauges.csv')
    gauge_data_path = os.path.join(PATH, '486/depth_width_sensitivity/reference_gauges')
    ...
    for i in range(num_sims):
        ...
        data = randb.arrange_data(new_breach, i)
        randb.combine_data(data)
        write_path = f'r{i}'
        if not os.path.exists(write_path):
            os.mkdir(write_path)
        randb.write_breach_data(6, write_path)
        copy_source = 'req_geoclaw'
        for file in os.listdir(copy_source):
            shutil.copy(os.path.join(copy_source, file), os.path.join(sim_dir, file))
