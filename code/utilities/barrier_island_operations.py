...
def create_island_mask(topo_data, points_indices, 
    ...
    np.savez(save_path + f'masked_{island_name}_1188x2484.npz', 
...
def create_static_breach(topo_data, breach_loc):
    """
    Runs through the barrier island and creates a breach at a specified
    location
    """
    x = topo_data.x.copy()
    y = topo_data.y.copy()
...
def close_inlet(topo_data, inlet_loc, name, save_path):
    """
    Adds land to the inlet region to close the inlet
    """
    w = inlet_loc['west']
    e = inlet_loc['east']
    n = inlet_loc['north']
...
if __name__ == '__main__':
    ...
    filter_region = [-72.88, -72.65, 40.72, 40.83] #[-72.885652,-72.634247,40.718299,40.828344] # Moriches Bay, NY
