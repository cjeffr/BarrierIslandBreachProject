...
def load_topo_data():
    topo_file = os.path.join('/projects/weiszr_lab/catherine/bathymetry/', 'moriches.nc') # /projects/weiszr_lab/catherine/bathymetry/
...
def runit(write_path, vary_width=False, vary_depth=False):
    ...
    for i in range(num_breaches):
        ...
        if vary_width and vary_depth:
           e, w, _ = randomize_width(lat, mu[i])
           _, _, d = randomize_depth(i)
           east.append(e)
           west.append(w)
           depth.append(d)
        if vary_width and not vary_depth:
            e, w, d = randomize_width(lat, mu[i])
            east.append(e)
            west.append(w)
            depth.append(d)
        if vary_depth and not vary_width:
    ...
    if num_breaches == 1:
        ...
        south = south[:num_breaches]
        north = north[:num_breaches]
        mu = mu[:num_breaches]
