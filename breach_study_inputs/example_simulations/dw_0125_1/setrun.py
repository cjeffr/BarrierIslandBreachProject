...
def get_points_from_bay(ds, lon, lat, distance_away, bearing):
    import math
    R = 6378.1
    bearing = math.radians(bearing)
    lat1 = math.radians(lat)
    lon1 = math.radians(lon)
    lat2 = math.asin(math.sin(lat1)*math.cos(distance_away/R) + 
                     math.cos(lat1)*math.sin(distance_away/R)*math.cos(bearing))
    lon2 = lon1 + math.atan2(math.sin(bearing)*math.sin(distance_away/R)*math.cos(lat1),
                             math.cos(distance_away/R)-math.sin(lat1)*math.sin(lat2))
...
#------------------------------
    ...
    # Initial time:
    # -------------
    storm_file = ('/projects/weiszr_lab/catherine/storm_files/NACCS_TP_0486_SYN_L2.nc')
    ...
    flagregion.t1 = time_start # days2seconds(0.416667)
    ...
    flagregion.t1 = time_start # days2seconds(0.0)
    flagregion.t2 = time_final # days2seconds(3.0)
    flagregion.spatial_region_type = 2
...
#-------------------
   ...
   # Sandy Hook
    ...
    data.storm_file = os.path.expandvars(os.path.join('/projects/weiszr_lab/catherine/storm_files','NACCS_TP_0486_SYN_L2.nc')) #os.path.expandvars(os.path.join(os.getcwd(),
                       #                               'sandy.storm'))
