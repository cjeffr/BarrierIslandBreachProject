...
def get_random_location(PATH):
    masked_island = os.path.join(PATH, 'east_masked_moriches.npz')
    ...
    south, north, lon = locate_topo_index(lats[0], lats[-1], random_lon)
    return {'south': (lats[0], south),
            'north': (lats[-1], north),
            'lon': (random_lon, lon)} 
...
def load_topography(topo_path):
    from clawpack.geoclaw import topotools
    topo = topotools.read_netcdf(topo_path)
    filter_region = [-72.885652,-72.634247,40.718299,40.828344]
    topo_data = topo.crop(filter_region)
    return topo_data
...
def random_num_breaches():
    num_breaches = np.random.randint(0,100) # number of total breach locations found to be viable
    return num_breaches


def get_depth():
    depth = np.random.uniform(0,-2.01)
    return depth
