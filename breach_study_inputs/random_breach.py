...
class RandomBreach(): 
    ...
    def check_location_viability(self, max_dune, nearest_gauges):
        ...
        cols_greater = (nearest_gauges >= x_percent).any()
        if cols_greater.any():
            time_to_exceed_x_percent = [br.first_greater(nearest_gauges, x_percent, col) for col in cols_greater.index]
    ...
    def randomize_location(self):
        ...
        gauge_data = br.load_gauge_data(tide_gauges.index.values, self.gauge_data_path)
        
        breach_start, breach_stop = self.check_location_viability(max_dune, gauge_data)
        if breach_start:
...
def runit(sim_num):
    ...
    breach_data_path = os.path.join(PATH, '300km_radius_sims/d_0001_6/breach.data')
    gauge_location_path = os.path.join(PATH, 'ocean_gauges.csv')
    gauge_data_path = os.path.join(PATH, 'geoclaw_changes_validation/no_breach_300k_km/_output/') #geoclaw_changes_validation/no_breach_no_calc_radii/_ouput')
    ...
    for direction in ['east']: 
        ...
        write_path = f'{direction}_{sim_num:04}_{len(new_breach)}'
